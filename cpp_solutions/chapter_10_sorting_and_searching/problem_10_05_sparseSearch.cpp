...
namespace chapter_10 {
    int sparseSearchHelper(const std::vector<std::string>& array, const std::string& query, int start, int end) {
        ...
        if (query >= array[start] && query <= array[midpoint]) {
            return sparseSearchHelper(array, query, start, (leftPotentialMidpoint < (midpoint - 1) ? leftPotentialMidpoint : (midpoint - 1)));
        }
        else {  // recurse right - do not include array elements that are known to be empty
            return sparseSearchHelper(array, query, (rightPotentialMidpoint > (midpoint - 1) ? rightPotentialMidpoint : (midpoint - 1)), end);
        }
    }
